...
=head1 PROJECT STATUS IS RELEASE CANDIDATE

Most of Form::Diva is unlikely to change, but hasn't been road-tested enough to know for sure that mistakes haven't been made, either in the code or in interpretation of HTML5 standards. It is also possible that there are html5 elements that should be baked in rather than left for the 'extra' field. 
...
Form::Diva is intented to be used with Templating systems, the HTML stuff is best left to the templating system. Telling form diva what you need in the Form Tag and whether you prefer to use a submit input or a button and what junk goes in them is only going to make it more complex, and make your application less readable because you will have replaced HTML with abstract perl code but not saved any space or labor. Or just use one of the other Form::Modules on CPAN. 

=head2 Where I use Form::Diva, in my Model, View or Controller?

Use it wherever works best for your coding style. 

You can put all of the diva code into a Perl Macro in Template::Toolkit or just use it as Embedded Perl in Mojo::Template. You can then use the include directive for your templating system to move that code to a shared file. Or define your form elements in the Controller if that is where it makes sense. Or 
